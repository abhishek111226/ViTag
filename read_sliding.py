...
def feature_compare(thr_id, dir, file_cnt, window, isSlide):
    ...
    retain =[];	
    if start>=end:
	print("thr_id returns NULL"+str(thr_id))
	return;
    img1 = cv2.imread(os.path.join(dir, '%d.png') % start)
    for i in range(start+1,end):
    	img2 = cv2.imread(os.path.join(dir, '%d.png') % i)
	print("thr_id calling score on "+str(start)+" and "+str(i)+" " +str(thr_id))
	score = feature_matching(img1,img2)
	print("thr_id calling score"+str(thr_id))
	...
	return 
    if len(scores) == 1:
	print("thr_id returns something"+str(thr_id))
    ...
    print(" Selected " + str(thr_id)+str(start+select)) 		
...
def feature_pass(dir, window):
    ...
    logging.info("Selected after basic " + str(len(selected)))	
    remaining = num_files % window	
    gc.collect()	
    logging.info(" Remaining starting " + str(remaining))	
    if remaining != 0 :
    ...
    for i in range(1,num_of_iterations, num_of_processors):
		#for j in range(i,i+num_of_processors):
		print "Running "+str(i)+" Out of"+str(num_of_iterations)	
		#	selected.append(feature_compare(j,dir,num_files, window, True))
